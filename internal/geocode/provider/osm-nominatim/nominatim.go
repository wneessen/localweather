...
func (n *Nominatim) Reverse(ctx context.Context, coords types.Coordinate) (types.Address, error) {
	...
	if _, err = n.http.GetWithTimeout(ctx, reverseAPIEndpoint, &result, query, nil, APITimeout); err != nil {
		return address, fmt.Errorf("failed to fetch reverse address details from Nominatim API: %w", err)
	}
...
func (n *Nominatim) Search(ctx context.Context, address string) (types.Coordinate, error) {
	...
	if _, err = n.http.GetWithTimeout(ctx, searchAPIEndpoint, &result, query, nil, APITimeout); err != nil {
		return coords, fmt.Errorf("failed to fetch address details from Nominatim API: %w", err)
	}
