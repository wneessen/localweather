	...
	t.Run("poll succeeds with different TPV results", func(t *testing.T) {
		...
		for _, tc := range tests {
			t.Run(tc.name, func(t *testing.T) {
				...
				if coords.Latitude != tc.lat {
					t.Errorf("expected latitude to be %f, got %f", tc.lat, coords.Latitude)
				}
				if coords.Longitude != tc.lon {
					t.Errorf("expected longitude to be %f, got %f", tc.lon, coords.Longitude)
				}
				if coords.Accuracy != types.Accuracy(tc.acc) {
					t.Errorf("expected accuracy to be %f, got %f", tc.acc, coords.Accuracy.Float64())
				}
				if coords.GPSMode != tc.mode {
					t.Errorf("expected mode to be %d, got %d", tc.mode, coords.GPSMode)
				}
			})
		}
	})
